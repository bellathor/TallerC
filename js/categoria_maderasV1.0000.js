    ...
    if (opcion == 'consultar_maderas') {
        ...
        fetch(urlEnvio, {
            ...
            .then(datos => {
                ...
                if (datos.maderas != null) {
                    var maderas = datos.maderas;
                    for (let x = 0; x < maderas.length; x++) {
                        setTimeout(Llenar_Lista, 100 * x, maderas[x], maderas.length);
                    }
                } else if (maderas.error == 'true') {
                    alert('Hubo un problema con la base de datos.!');
                }

            })
            .catch(error => console.error("Error encontrado: ", error));
    } else if (opcion == 'consultar_madera_stock') {
            ...
            .then(datos => {
                if (datos.stock != null) {
                    ...
                    for (let x = 0; x < stock .length; x++) {
                        setTimeout(Llenar_Formulario, 100 * x, stock [x], stock .length);
                    }
                }
            })
            .catch(error => console.error("Error encontrado: ", error));
...
function Seleccion_Materiales(seleccion) {
    ...
    if (seleccion.value != "Seleccionar") {
        Fetch_GET(seleccion.value, 'consultar_madera_stock');
