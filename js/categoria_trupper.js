...
function Cargar_Hilos() {
    Fetch_GET(null, 'consultar_truppers');
}
function Fetch_GET(id, opcion) {
    var url = '../../php/scripts/apis/bodega_taller/api_trupper.php';

    if (opcion == 'consultar_truppers') {
    ...
    } else if (opcion == 'consultar_trupper') {
        ...
        fetch(urlEnvio, {
            ...
            .then(datos => {
                if (datos.reportes != null) {
                    ...
                    alert('No hay reportes de ferreteria.!');
                }
            })
            .catch(error => console.error("Error encontrado: ", error));
    }
...
function Seleccion_Materiales(seleccion) {
    ...
    if (seleccion.value != "Seleccionar") {
        Fetch_GET(seleccion.value, 'consultar_trupper');
